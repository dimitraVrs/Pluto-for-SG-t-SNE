...
This is a Pluto notebook inspired by the web site [How to Use t-SNE Effectively](https://distill.pub/2016/misread-tsne/) to help a user explore the visualization of high-dimensional data, using [SG-T-SNE-Pi](https://t-sne-pi.cs.duke.edu) more effectively.
...
md"""
Select the distance between two sets of points that form two parallel lines

0.0 $(@bind displacement PlutoUI.Slider(0.0:0.01:0.5)) 0.5
...
begin
	nE=200
	Random.seed!(123)
	XE=Random.rand(Normal(0,1),nE,nE)
	for j=1:nE
		XE[:,j]=XE[:,j]/j
	end
...
# ╔═╡ eeb451ed-2e99-4766-8290-b8d64a5998c8
md" Select algorithm parameters! "

# ╔═╡ 06bd3245-05da-4478-b390-d7406842222b
@bind rangeE Radio(["λ < 1", "λ > 1"], default="λ > 1")

# ╔═╡ d9102941-561a-46aa-9984-fb1aab428893
begin
	rangeE
	if cmp(rangeE,"λ > 1")==0
		valuesE=2:200
...
# ╔═╡ bde80532-7412-40be-8ec2-1e372fc85d83
begin
	rangeE
	md"""
	λ:
...
# ╔═╡ 059e5261-6314-498a-b9ee-fe068ec071f8
begin
	lambdaE
	md" λ selected= $lambdaE "
end

# ╔═╡ 721a3569-2206-4c1f-8c73-2bc199787dde
md"""
perplexity:
5 $(@bind uE PlutoUI.Slider(5:50)) 50
"""

# ╔═╡ 6ab6201e-0ea8-43aa-971c-4b021d1916a0
...
# ╔═╡ 262b1121-59ba-44d1-8955-d4fbab115b31
begin
	...
	Pg = hcat(gx[:], gy[:])
	Lg = collect(1:n^2)
	CairoMakie.scatter(Pg[:,1],Pg[:,2]; color=Lg)
end
...
begin
	Yg = sgtsnepi(pointcloud2graph(Pg,u),λ=lambda)
	CairoMakie.scatter(Yg[:,1],Yg[:,2]; color=Lg)
end

# ╔═╡ a78909c7-4ad2-47ea-98ad-7c01358ef98d
md"""
...
begin
	n0=200
	Random.seed!(123)
	X1Size=Random.rand(Normal(0,1),n0,2)

	X2Size=X1Size
	X2Size[:,1]=X2Size[:,1].+100

	X1Size=10*X1Size

	XSize=vcat(X1Size,X2Size)
...
# ╔═╡ 807a0dd3-a1c0-4de9-baab-0fd5ee843ff8
md" Select algorithm parameters! "

# ╔═╡ 2814064a-bb4e-4b88-889a-5a605ea3b81a
@bind rangeSize Radio(["λ < 1", "λ > 1"], default="λ > 1")

# ╔═╡ 682d0a9a-ba14-4816-9d7a-0dfa47a588b4
begin
	rangeSize
...
# ╔═╡ 897c59a4-9651-42b4-8f24-d35ebadfe480
begin
	rangeSize
	md"""
	λ:
...
# ╔═╡ d64d2803-8109-4eb2-8a45-c9c1b525b5e1
begin
	lambdaSize
	md" λ selected= $lambdaSize "
end

# ╔═╡ c4cf7a0a-e878-42c5-b7ce-f4d94ebe1863
md"""
perplexity:
5 $(@bind uSize PlutoUI.Slider(5:50)) 50
"""

# ╔═╡ 1449922c-0b45-467c-90e9-4f06db4c716c
md" perplexity selected= $uSize "

# ╔═╡ 0d2b043b-eb19-490c-9528-49a34ab40932
begin
	YSize = sgtsnepi(pointcloud2graph(XSize,uE),λ=lambdaSize)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "59dd90852f102349b00cf750488a431de54cf9c4"
