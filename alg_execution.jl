...
begin
	...
	if dataChoice==choices[1]
		sectionData="differentSizes"
	elseif dataChoice==choices[2]
		sectionData="differentDistances"
	elseif dataChoice==choices[3]
		sectionData="sameDistance"
	elseif dataChoice==choices[4]
		sectionData="concentricCircles"
	elseif dataChoice==choices[5]
		sectionData="trefoil"
	elseif dataChoice==choices[6]
		sectionData="linkedRings"
	elseif dataChoice==choices[7]
		sectionData="unlinkedRings"
	elseif dataChoice==choices[8]
		sectionData="ellipse"
	elseif dataChoice==choices[9]
		sectionData="parallelLines"
	elseif dataChoice==choices[10]
		sectionData="noise"
	elseif dataChoice==choices[11]
		sectionData="circle"
	elseif dataChoice==choices[12]
		sectionData="randomCircle"
	elseif dataChoice==choices[13]
		sectionData="orthogonalSteps"
	elseif dataChoice==choices[14]
		sectionData="randomWalk"
...
@bind demo Radio(["demo_stochastic_matrix", "demo_perplexity_equalization"], default="demo_stochastic_matrix")

# ╔═╡ f331562a-a104-49aa-af26-bdff42d4427f
md" demo selected = $demo "
...
function callExec()
	...
	if cmp(demo,"demo_stochastic_matrix")==0
		...
		option1="-u$u"
		name=string(sectionData,"-demo-perpl")
	end
...
function showPlots(y,graph)
		...
		if cmp(demo,"demo_stochastic_matrix")==0
			...
			points = MatrixMarket.mmread("$(graph)-demo-perpl.mtx")
		end
		...
		anim=@animate for i in 2:50:iterations
				...
				elseif (graph=="linkedRings"||graph=="unlinkedRings"||graph=="differentSizes"||graph=="concentricCircles"||graph=="parallelLines"||graph=="twoRings")
					...
					# two-cluster plot
					Plots.scatter(y[iterStart:end1,1],y[iterStart:end1,2],title="points after iteration $i",label="cluster 1",markercolor=:blue,markersize = 3)

					Plots.scatter!(y[start2:iterEnd,1],y[start2:iterEnd,2],title="points after iteration $i",label="cluster 2",markercolor=:orange,markersize = 3)

					scatter!(legend=:outerbottom, legendcolumns=2)
					
				elseif(graph=="sameDistance"||graph=="differentDistances")

					# three-cluster plot
					Plots.scatter(y[iterStart:end1,1],y[iterStart:end1,2],title="points after iteration $i",label="cluster 1",markercolor=:blue,markersize = 3)

					Plots.scatter!(y[start2:end2,1],y[start2:end2,2],title="points after iteration $i",label="cluster 2",markercolor=:orange,markersize = 3)

					Plots.scatter!(y[start3:iterEnd,1],y[start3:iterEnd,2],title="points after iteration $i",label="cluster 3",markercolor=:green,markersize = 3)

					scatter!(legend=:outerbottom, legendcolumns=3)
		...
		elseif (graph=="linkedRings"||graph=="unlinkedRings"||graph=="differentSizes"||graph=="concentricCircles"||graph=="parallelLines"||graph=="twoRings")
					...
					p=Plots.scatter!(y[start2:iterEnd,1],y[start2:iterEnd,2],title="points after iteration $i",label="cluster 2",markercolor=:orange,markersize = 3)
		 ...
		 elseif(graph=="sameDistance"||graph=="differentDistances")
					...
					p=Plots.scatter!(y[start3:iterEnd,1],y[start3:iterEnd,2],title="points after iteration $i",label="cluster 3",markercolor=:green,markersize = 3)
...
begin
		...
		if cmp(demo,"demo_stochastic_matrix")==0
			...
			mp4(showPlots(yS,sectionData),"$(sectionData)_u$(u)_iter$(iterations).mp4",fps=1,show_msg=false)
		end
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "88aecbde6550ed0d907255fe9189c9e13c105861"
